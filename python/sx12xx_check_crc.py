...
class sx12xx_check_crc(gr.basic_block):
    ...
    def handle_msg(self, msg_pmt):
        ...
        if res == packet_crc:
            ...
            # F4TNK It#5: 1-bit-flip CRC-16 retry
    ...
    def _try_1bit_flip(self, packet, expected_crc):
        """F4TNK It#5: try flipping each bit in payload (excl. CRC trailer)."""
        packet = bytearray(packet)
        for byte_idx in range(len(packet) - 2):
            for bit_idx in range(8):
                packet[byte_idx] ^= (1 << bit_idx)
                if crc16(packet[:-2], self.initial, self.final) == expected_crc:
                    return bytes(packet)
                packet[byte_idx] ^= (1 << bit_idx)
        return None
